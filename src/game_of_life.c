...
int32_t is_neighbor_in_field(int32_t row_idx, int32_t col_idx)
{
...
int32_t main(void)
{
    int32_t matrix[NUM_ROWS][NUM_COLS];
    int32_t row_idx = 0;
    int32_t col_idx = 0;
    int32_t idx_sum = 0;
    int32_t num_live_neighbors = 0;
    ...
    for (generation = 0; generation < NUM_GENERATIONS; generation++) {
        for (row_idx = 0; row_idx < NUM_ROWS; row_idx++) {
            for (col_idx = 0; col_idx < NUM_COLS; col_idx++) {
                ...
                if (is_neighbor_in_field(row_idx - 1, col_idx - 1) == 1) {
