    ...
    begin
        ...
        test_cases_loop : while test_suite loop
            ...
            if run("test_outputs_are_correct_when_reset_is_enabled") then
                ...
                address_in             <= std_logic_vector(to_signed(111, 32));
                wait for CLK_PERIOD * 2;
                check_equal(instruction, std_logic_vector(to_signed(0, 32)));
                check_equal(address_out, std_logic_vector(to_signed(0, 32)));
                check_sig              <= 1;
                ...
                write_trig             <= '1';
                halt                   <= '1';
                data_word_from_uart    <= std_logic_vector(to_signed(45, 32));
                address_word_from_uart <= std_logic_vector(to_signed(200, 32));
                address_in             <= std_logic_vector(to_signed(111, 32));
                wait for CLK_PERIOD * 2;
                reset                  <= '0';
                wait for CLK_PERIOD * 2;
                data_word_from_uart    <= std_logic_vector(to_signed(22, 32));
                address_word_from_uart <= std_logic_vector(to_signed(100, 32));
                wait for CLK_PERIOD * 2;
                wait for CLK_PERIOD * 2;
                check_equal(instruction, std_logic_vector(to_signed(0, 32)));
                check_equal(address_out, std_logic_vector(to_signed(0, 32)));
                check_equal(prog_mem(100), std_logic_vector(to_signed(22, 32)));
                check_equal(prog_mem(200), std_logic_vector(to_signed(45, 32)));
                check_sig              <= 1;
                info("===== TEST CASE FINISHED =====");
                ...
                data_word_from_uart    <= (others => '0');
                address_word_from_uart <= (others => '0');
                prog_mem(0)            <= force std_logic_vector(to_signed(15, 32));
                prog_mem(45)           <= force std_logic_vector(to_signed(389, 32));
                wait for CLK_PERIOD * 2;
                reset                  <= '0';
                fetch_enable           <= '1';
                address_in             <= std_logic_vector(to_signed(0, 32));
                wait for CLK_PERIOD * 2;
                check_equal(instruction, std_logic_vector(to_signed(15, 32)));
                check_equal(address_out, std_logic_vector(to_signed(0, 32)));
                wait for CLK_PERIOD;
                address_in             <= std_logic_vector(to_signed(20, 32));
                wait for CLK_PERIOD * 2;
                check_equal(instruction, std_logic_vector(to_signed(0, 32)));
                check_equal(address_out, std_logic_vector(to_signed(20, 32)));
                wait for CLK_PERIOD;
                address_in             <= std_logic_vector(to_signed(45, 32));
                wait for CLK_PERIOD * 2;
                check_equal(instruction, std_logic_vector(to_signed(389, 32)));
                check_equal(address_out, std_logic_vector(to_signed(45, 32)));
                check_sig              <= 1;
                info("===== TEST CASE FINISHED =====");
            end if;
