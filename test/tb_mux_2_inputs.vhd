    ...
    begin
        ...
        test_cases_loop : while test_suite loop
            ...
            if run("test_output_is_zero_if_reset_is_enabled") then
                ...
                check_sig <= 1;
                info("===== TEST CASE FINISHED =====");
            elsif run("test_control_switch_from_zero_to_one") then
                info("--------------------------------------------------------------------------------");
                info("TEST CASE: test_control_switch_from_zero_to_one");
                info("--------------------------------------------------------------------------------");
                reset     <= '1';
                wait for CLK_PERIOD * 2;
                reset     <= '0';
                ...
                input_2   <= std_logic_vector(to_unsigned(200, 32));
                wait for CLK_PERIOD * 2;
                check_equal(output, input_1);
                control   <= '1';
                wait for CLK_PERIOD * 2;
                ...
                check_sig <= 1;
                info("===== TEST CASE FINISHED =====");
            elsif run("test_control_switch_from_one_to_zero") then
                info("--------------------------------------------------------------------------------");
                info("TEST CASE: test_control_switch_from_one_to_zero");
                info("--------------------------------------------------------------------------------");
                reset     <= '1';
                wait for CLK_PERIOD * 2;
                reset     <= '0';
                ...
                check_sig <= 1;
                info("===== TEST CASE FINISHED =====");
            elsif run("test_output_with_all_ones_input") then
                info("--------------------------------------------------------------------------------");
                info("TEST CASE: test_output_with_all_ones_input");
                info("--------------------------------------------------------------------------------");
                reset     <= '1';
                wait for CLK_PERIOD * 2;
                reset     <= '0';
