...
tb_lib.add_source_files([os.path.join(src_dir, "*uart_package.vhd")])
...
if tb_name:
    if "tb_program_counter" in tb_name:
        ...
        VU.set_sim_option(
            "modelsim.vsim_flags.gui", ["-lib design_lib", f"-do {test_dir}/tb_alu_vsim.txt"]
        )
    elif "tb_pc_adder" in tb_name:
        VU.set_sim_option(
            "modelsim.vsim_flags.gui", ["-lib design_lib", f"-do {test_dir}/tb_pc_adder_vsim.txt"]
        )
        ...
        VU.set_sim_option(
            ...
            [
                "-lib design_lib",
                f"-do {test_dir}/tb_riscv_vsim.txt",
                f"-ginput_file={SCRIPT_DIR}/riscv_stimulus.txt",
            ],
        )
    elif "tb_clk_div" in tb_name:
        VU.set_sim_option(
            "modelsim.vsim_flags.gui", ["-lib design_lib", f"-do {test_dir}/tb_clk_div_vsim.txt"]
        )
    elif "tb_writeback_mux" in tb_name:
        VU.set_sim_option(
            "modelsim.vsim_flags.gui",
    ...
    elif "tb_uart" in tb_name:
        VU.set_sim_option(
            "modelsim.vsim_flags.gui", ["-lib design_lib", f"-do {test_dir}/tb_uart_vsim.txt"]
        )
